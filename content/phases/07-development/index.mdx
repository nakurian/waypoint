...
**Good.** "This PR adds a composite index on (user_id, order_id) to orders.items. The read path for /v1/orders/summary does a double lookup on these columns per request, and our p95 at peak hours is now over 800ms. Index size is modest (~150MB on current table), so the write-amp cost is acceptable. No behavioural change — read semantics identical."
</Callout>
