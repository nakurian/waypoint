...
describe('<CopyButton>', () => {
  ...
  it('copies the provided text to clipboard on click', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith('npx waypoint-claude init --role=developer');
  });
  ...
  it('falls back to selectable text when clipboard API is unavailable', async () => {
